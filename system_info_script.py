...
if __name__ == "__main__":
    ...
    if ip_address is not None and mac_address is not None and hostname is not None and disk_space is not None and memory_usage is not None and cpu_usage is not None:
        ...
        # Define o caminho completo para o arquivo de saída no mesmo diretório
        output_file_path = os.path.join(script_directory, "info_system.txt")

        with open(output_file_path, "a") as file:
            file.write(info_text)
        print(f"As informações foram salvas no arquivo \n'{output_file_path}'.")
